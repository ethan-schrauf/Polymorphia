...
function getGame() {
    ...
    xhr.onload = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            document.getElementById("playAutomatically").checked = false;
...
function setGameTitle(name) {
    const statusElement = document.getElementById("gameTitle");
...
function populateGameDropDown() {
    ...
    xhr.onload = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            justPopulateGameDropDown(gameIds);
        }
    };
    xhr.send();
}

function justPopulateGameDropDown(gameIds) {
    const gameDropDown = document.getElementById("gameDropDown");
...
function createTournament() {
    createRound("api/tournament/create")
}

function createNextRound(url) {
    createRound("api/tournament/createRound")
}

function createRound(url) {
    const xhr = new XMLHttpRequest();
    xhr.open("POST", url, true);
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.onload = function () {
        if (xhr.readyState === 4 && xhr.status === 201) {
            ...
            justPopulateGameDropDown(payload.gameIds);
